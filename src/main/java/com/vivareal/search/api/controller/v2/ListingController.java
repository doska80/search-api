...
public class ListingController {
    ...
    public SearchApiResponse getListings(SearchApiRequest request) {
        ...
        SearchRequestBuilder inmuebles = client.prepareSearch("inmuebles");
        inmuebles.setQuery(boolQuery);

        SearchResponse searchResponse = inmuebles.execute().actionGet();

        SearchApiResponse searchApiResponse = new SearchApiResponse();

        searchResponse.getHits().forEach(doc -> searchApiResponse.addList(doc.getSource()));

        return searchApiResponse;
    }

    @RequestMapping("/stream")
    public void stream(SearchApiRequest request, HttpServletResponse response) throws IOException {
        ...
        ResponseStream.create(response.getOutputStream())
                .withIterable(searchResponse.getHits(), SearchHit::source);
    }
