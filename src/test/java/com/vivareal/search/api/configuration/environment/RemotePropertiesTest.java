...
public class RemotePropertiesTest {
    ...
    public void validateGetValueForPropertiesAsSet() {
        PROPERTIES_AS_SET.forEach(property -> {
    ...
    public void checkIsValidRequestValueForNumericProperties() {
        NUMERIC_PROPERTIES.forEach(property -> {
            ...
            // Test over custom index
            assertEquals(property.name(), numberAsInteger(MAX_VALUE), numberAsInteger(property.getValue(null, CUSTOM_INDEX)));
            assertEquals(property.name(), numberAsInteger(0), numberAsInteger(property.getValue(0, CUSTOM_INDEX)));

            // Test over default index
            assertEquals(property.name(), numberAsInteger(MIN_VALUE), numberAsInteger(property.getValue(null, DEFAULT_INDEX)));
            assertEquals(property.name(), numberAsInteger(0), numberAsInteger(property.getValue(0, DEFAULT_INDEX)));
    ...
    private void checkIsValidRequestForProperty(RemoteProperties property, Set<String> expectedIndexValue, String indexName) {
        assertEquals(property.name(), expectedIndexValue, property.getValue(null, indexName));
        assertEquals(property.name(), expectedIndexValue, property.getValue(new HashSet<>(), indexName));
        assertEquals(property.name(), newLinkedHashSet("requestValue"), property.getValue(newLinkedHashSet("requestValue"), indexName));
    }
}
