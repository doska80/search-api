...
public class StandardDatasetAsserts {
    ...
    @Autowired
    public StandardDatasetAsserts(@Value("${itest.standard.dataset.size}") Integer standardDatasetSize,
    ...
    private <T> List<T> getIds(IntStream intStream, Predicate<Integer> filter, int limit, Function<Integer, T> function) {
        return intStream
            .boxed()
            .filter(filter)
            .limit(limit)
