...
public class SearchByOperatorInIntegrationTest extends SearchApiIntegrationTest {
  ...
  public void validateInFilterOnSingleField() {
    given()
        ...
        .body(
            ...
            equalTo(of(1, standardDatasetSize).collect(toList())));
  }
  ...
  public void validateInFilterOnArrayField() {
    ...
    given()
        ...
        .get(
            TEST_DATA_INDEX
                + "?filter=object.object.array_string IN ["
                + range.stream().map(i -> format("\"%d\"", i)).collect(joining(","))
                + "]")
        .then()
  ...
  public void validateInFilterOnArrayFieldWhenNested() {
    ...
    given()
        ...
        .get(
            TEST_DATA_INDEX
                + "?filter=nested.object.array_string IN ["
                + range.stream().map(i -> format("\"%d\"", i)).collect(joining(","))
                + "]")
        .then()
        .body("totalCount", equalTo(range.size()))
  ...
  public void validateSearchByIdsUsingInOperator() {
    given()
        ...
        .get(
            TEST_DATA_INDEX
                + "?sort=id ASC&includeFields=id&filter=id IN [1, "
                + standardDatasetSize
                + "]")
        .then()
        .body("totalCount", equalTo(2))
        .body("result.testdata", hasSize(2))
        .body(
            "result.testdata.id", equalTo(of("1", valueOf(standardDatasetSize)).collect(toList())));
  }
}
