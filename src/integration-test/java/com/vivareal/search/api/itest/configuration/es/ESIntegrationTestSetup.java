...
public class ESIntegrationTestSetup {
    ...
    private void readConfigurationFile() {
        try {
            ...
            this.boostrapConfiguration = new ObjectMapper().readValue(getBoostrapConfig(), new TypeReference<Map<String, Object>>() {});
        } catch (IOException e) {
    ...
    private String getBoostrapConfig() throws IOException {
        ...
        try (BufferedReader buffer = new BufferedReader(new InputStreamReader(getClass().getResourceAsStream(INDEXES_FILE)))) {
    ...
    private void executeSingleCommand(Map<String, Object> req) {
        ...
        Header[] headers = ofNullable(req.get("header"))
            .map(item -> (List<Map<String, String>>) item).orElse(emptyList())
            .stream()
