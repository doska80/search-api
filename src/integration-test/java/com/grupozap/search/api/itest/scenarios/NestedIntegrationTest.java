...
public class NestedIntegrationTest extends SearchApiIntegrationTest {
  ...
  public void validateComplexQueryUsingNestedAndNoNestedFieldsRecursively() {
    given()
        ...
        .get(
            TEST_DATA_INDEX
                + "?filter=object.object.field IN ['common'] AND facetString:'A' AND (nested.object.field:'common' AND nested.id>=10 AND nested.id<=20)")
        .then()
        .body(
            "result.testdata.nested.id.sort()",
            equalTo(rangeClosed(10, 18).boxed().collect(toList())))
