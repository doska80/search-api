node {
    ...
    withCredentials([
        ...
        def command = "make ENV=${env.ENV} DEPLOY_GROUP=${env.DEPLOY_GROUP} APP=${env.APP} IMAGE_NAME=${env.IMAGE_NAME} K8S_CLUSTER=${k8s_cluster} K8S_TOKEN=${K8S_TOKEN} STACK_ALIAS=${env.STACK_ALIAS} SLK_TOKEN=${SLK_TOKEN} DATADOG_ENABLED=${env.DATADOG_ENABLED} ES_CLUSTER_NAME=${env.ES_CLUSTER_NAME} ES_HOSTNAME=${hostname} ${env.OPERATION}-with-notification"
...
def sendToSlack(result, imageName, environment) {
    ...
    slackSend channel: '#notifs-matching', color: color, message: "${message}", teamDomain: 'grupozap'
}
