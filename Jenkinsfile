node {
    ...
    withCredentials([
        ...
        def taskAction = env.TASK_ACTION
        def command = "make ENV=${env.ENV} AWS_DEFAULT_REGION=${env.AWS_DEFAULT_REGION} IMAGE_NAME=${env.IMAGE_NAME} STACK_ALIAS='${env.ES_CLUSTER_NAME}-${env.STACK_ALIAS}' SLK_TOKEN=${SLK_TOKEN} ES_CLUSTER_NAME=${env.ES_CLUSTER_NAME} NEWRELIC_ENABLED=${env.NEWRELIC_ENABLED} deploy-with-notification"
